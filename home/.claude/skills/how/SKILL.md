...
Spawn all explorers in a single message (one Agent call per explorer, all in the same block):
...
After the explanation is complete, spawn one architectural critic per row below, all in a single message. Independent perspectives are the point, so keep the model families distinct:

| Critic | How to spawn |
|--------|--------------|
| A | `subagent_type: general-purpose`, `model: fable` |
| B | `subagent_type: general-purpose`, `model: opus` |
| C | gpt-5.5 via Codex (see below) |
