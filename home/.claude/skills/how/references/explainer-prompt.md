...
**This is read-only work.** Do not edit or create files, and do not run mutating commands. You have the tools to write; you must not use them. Within that limit, check anything, clarify a detail, or fill a gap using Read, Grep, and Glob. The explorers did the heavy lifting, so you shouldn't need to re-explore from scratch.
